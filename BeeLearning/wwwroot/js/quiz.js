...
const questionCountElement = document.getElementById('question-count'); // Elemento para mostrar quantas perguntas faltam

let currentQuestionIndex = 0;
let score = 0;
let maxQuestions = 10;
let selectedQuestions = [];
let availableQuestions = []; // Array para manter as perguntas disponíveis
...
function startGame() {
    ...
    availableQuestions = [...questions]; // Clona o array de perguntas

    selectedQuestions = shuffleArray(availableQuestions).slice(0, maxQuestions); // Embaralha as perguntas e seleciona as perguntas
...
function setNextQuestion() {
    ...
    updateQuestionCount(); // Atualiza o contador de perguntas restantes
}
...
function selectAnswer(answer) {
    ...
    nextButton.classList.remove('hide'); // Mostra o botão "PRÓXIMA" somente após a resposta
}
...
// Nova função para atualizar o contador de perguntas restantes
function updateQuestionCount() {
    const questionsLeft = maxQuestions - currentQuestionIndex - 1; // Calcula quantas perguntas faltam
    questionCountElement.innerText = `Faltam ${questionsLeft} perguntas`; // Atualiza o texto do contador
}
