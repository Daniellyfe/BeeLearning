...
function avaliarRedacao() {
    ...
    if (redacao.length === 0) {
        ...
        const palavrasTese = ['portanto', 'é necessário', 'deve-se', 'é essencial', 'é importante', 'é preciso', 'urge que', 'é indispensável', 'torna-se fundamental', 'é imperativo', 'exige-se', 'é crucial', 'implica em', 'é relevante', 'não se pode ignorar que', 'requer-se', 'é vital', 'torna-se prioritário', 'é de suma importância'];
        ...
        const palavrasRepertorio = ['história', 'cultura', 'política', 'governo', 'sociedade', 'ciência', 'economia', 'Djamila Ribeiro', 'John Locke', 'Chimamanda Adichie'];
        ...
        const palavrasArgumentacao = ['porque', 'portanto', 'assim', 'logo', 'então', 'por conseguinte', 'dessa forma', 'ou seja', 'em síntese', 'diante disso', 'por isso', 'por outro lado', 'de acordo com', 'ademais', 'além disso', 'de fato', 'entretanto', 'apesar disso', 'no entanto', 'considerando que', 'é importante ressaltar que', 'devemos considerar que', 'outro ponto relevante é que', 'a partir disso, podemos concluir que', 'um exemplo que ilustra essa situação é', 'cabe destacar que'];
        ...
        const palavrasConclusao = ['em conclusão', 'portanto', 'conclui-se que', 'assim', 'em resumo', 'para finalizar', 'por fim', 'em suma', 'dessa forma', 'por isso', 'de modo geral', 'cabe ressaltar que', 'para encerrar', 'em face do exposto', 'em síntese', 'de acordo com o que foi discutido', 'assim sendo', 'para concluir', 'por conseguinte', 'com isso', 'diante do exposto'];
    ...
    // Se a redação contém todos os elementos adequados (tese, argumentação, repertório sociocultural, conclusão), garantir nota 1000
    if (pontuacao >= 1000 && feedback.length === 0) {
